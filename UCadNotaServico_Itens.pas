...
begin
  ...
  begin
    if vIDServicoAnt <> RxDBLookupCombo1.KeyValue then
...
procedure TfrmCadNotaServico_Itens.memoDescricaoEnter(Sender: TObject);
begin
  memoDescricao.SelStart := length(memoDescricao.Text);
end;

procedure TfrmCadNotaServico_Itens.memoDescricaoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmCadNotaServico_Itens.Panel2Enter(Sender: TObject);
begin
  vIDServicoAnt := fDMCadNotaServico.cdsNotaServico_ItensID_SERVICO_INT.AsInteger;       
end;
